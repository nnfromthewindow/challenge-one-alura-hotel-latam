...
public class RegistroHuesped extends JFrame {
	...
	public RegistroHuesped(int idReserva) {
	    ...
	    c.setTime(hoy);
	    c.add(Calendar.YEAR, -edadMinima);
	    hoy = c.getTime();
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if(	!txtNombre.getText().isBlank() &&
						...
						txtTelefono.getText().matches("(^\\d*)") &&
						txtNombre.getText().matches("(^\\D{0,20})") &&
						txtApellido.getText().matches("(^\\D{0,20})")
							) {
						...
						if(txtTelefono.getText().isBlank()||!txtTelefono.getText().matches("(^\\d*)")) {
							JOptionPane.showMessageDialog(null, "El numero de telefono debe contener solo numeros sin espacios en blanco");	
						}
